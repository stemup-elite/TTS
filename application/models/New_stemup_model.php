...
Class New_stemup_model extends CI_Model
{
    ...
    function get_list_news($limit=6,$page){
        ...
        $sql="SELECT * FROM savsoft_news WHERE status=1 ORDER BY id desc LIMIT $limit offset ".($page*$limit);
    ...
    function get_other_news_rand1(){
        $sql="SELECT * FROM savsoft_news ORDER BY modify_date desc LIMIT 3";
        $query=$this->db->query($sql);
        if($query->num_rows() > 0){
            return $query->result_array();
        }else
    ...
    function get_common_news_rand(){
     ...
     function get_common_news_rand_1(){
        $sql="SELECT * FROM savsoft_news WHERE featured=1 LIMIT 3";
        $query=$this->db->query($sql);
        if($query->num_rows() > 0){
            return $query->result_array();
        }else
