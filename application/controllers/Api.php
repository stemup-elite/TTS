...
class Api extends CI_Controller {
    ...
    public function api_connect3($api_key='savsoft'){
       
		$email=$_POST['email'];
		$password=$_POST['password'];

		log_message("error",'--->Login: email: '.$email);
        ...
        if($ur['status']=='1'){     
			...
			log_message("error",'Login Successfull');
        }else{
            
            
            
            $date=date("Y-m-d");
			$err = "\n"."ERROR - ".date('Y-m-d H:i:s')."-->LOGIN: -->";
			$login_day = 'login-'.$date.'.php';

            if($ur['status']=='0'){
            	$status=0;
            	$data1z = $err.'Tài khoản đăng nhập không chính xác: '.$email;
            	log_message('error','Error: ---> '.$status.': Tai khoan dang nhap khong chinh xac');

            }else if($ur['status']=='2'){
            	$status=2;
            	$data1z =$err.'Tài khoản chưa được xác thực: '.$email;
            	log_message('error','Error: ---> '.$status.': Tai khoan chua duoc xac thuc');

            }else if($ur['status']=='3'){
            	$status=3;
            	$data1z = $err.'Tài khoản đang ở trạng thái Inactive: '.$email;
            	log_message('error','Error: ---> '.$status.': Tai khoan dang o trang thai Inactive');

            }
            	// write_file(base_url('./application/login_logs/').$login_day, $data1z,'a+');
			//if ( !write_file('./application/login_logs/'.$login_day, $data1z,'a+')){
	...
	function result_list_01($connection_key='',$uid='',$limit='0'){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$limit=$_POST["limit"];
		...
		$user=$auth->row_array();
		if($auth->num_rows()==0){
			exit(json_encode(array('message'=>'invalid Connection!')));
		} else {
			$this->db->where('uid',$uid);
			$this->db->where('connection_key',$connection_key);
			$this->db->join('account_type','account_type.account_id=savsoft_users.su');
		...
		if($data['quiz']['video_mcq']){
			...
			if(count($vidTags[0])>0){
						 ...
						 if(strpos($match[1],'youtube')!==false){
							...
							$vidlink="https://www.facebook.com/plugins/video.php?href=https%3A%2F%2Fwww.facebook.com%2Fthedadlab%2Fvideos%2F".$vid."%2F";
	...
	function load_quiz_nguoitot_01(){	
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$quid=$_POST["quid"];
		//Check connection_key
					...
					preg_match_all('/<iframe[^>]+>/i',$a['question'], $vidTags); 
							if(count($vidTags[0])>0){
								 ...
								 if(strpos($match[1],'youtube')!==false){
									...
									$posid=strpos($temp,"%2F");
									$vid= substr($temp,0,$posid);
									$data['video']['link']="https://www.facebook.com/plugins/video.php?href=https%3A%2F%2Fwww.facebook.com%2Fthedadlab%2Fvideos%2F$vid%2F";
									$img_video="https://graph.facebook.com/$vid/picture";
...
function signup4(){
		$user_info = json_decode($this->security->xss_clean($this->input->raw_input_stream),true);
		//Phu huynh
		$email_ph = $user_info['email_ph'];
		$first_name_ph = $user_info['first_name_ph'];
		$password_ph= $user_info['password_ph'];
		$this->db->where('email', $email_ph);
		$n=$this->db->get('savsoft_users')->num_rows();
		$su_ph=$user_info['su_ph'];
		//log_message('error', "zzzzzzzzzzz@@".json_encode($user_info));
		if($su_ph==4){
			$point=10000;
		}
		...
		if($n==0){
			...
			while($check){
				$user_code_ph = '';
				for ($i = 0; $i < 6; $i++) {
					$user_code_ph.= $characters[rand(0, $charactersLength - 1)];
				}
				$this->db->where('user_code', $user_code_ph);
				$m = $this->db->get('savsoft_users')->num_rows();
				if($m==0)
					$check=false;
			}	
			...
			$post_ph = array(
				'first_name'=>$first_name_ph,
				'last_name'=>'',
				'email'=>$email_ph,
				'email2'=>$email_ph,
				'password'=>md5($password_ph),
				'gid'=>5,
				'su'=>$su_ph,
				'subscription_expired'=>strtotime('2030-12-31'),
				'user_code'=>$user_code,
				...
				$result_ph = $this->db->insert('savsoft_users', $post_ph);	
				$parent_id= $this->db->insert_id();
		
		$uidp=$parent_id;
		$password=$user_info['password_hs'];
		$name = $user_info['first_name_hs'];
		$email = $user_info['email_hs'];
		$grade = $user_info['grade_hs'];
		
		
		$data_insert_hs=$this->user_model->insert_user_6($uidp,$password, $name,$email, $grade);
		log_message('error','---------Insert vao DB');
		$child_id= $data_insert_hs['childid'];
		log_message('error','---------'.$child_id);
		//echo json_encode($data_insert_hs);
		if($data_insert_hs['message']=="success"){
		    $childidx = $data_insert_hs['childid'];
			for($i=0; $i<3;$i++){
				$distince_time="-$i day";
			...
			if($data_insert_hs && $result_ph){
				echo json_encode(array("message"=>"success", "parentid"=>$parent_id, "childid"=>$child_id));
			}
			else{
				echo json_encode(array("message"=>"error"));
			}
		...
		if($result_ph){
			...
			//send email
				...
				$this->email->initialize($config);	
					$fromemail=$this->config->item('fromemail');
					$fromname=$this->config->item('fromname');
					if (filter_var($email_ph, FILTER_VALIDATE_EMAIL)) {
						$subject="Chúc mừng bạn trở thành thành viên Stemup.app";
						$signup_name = $first_name_ph;
						$signup_email = $email_ph;
...
function signup5(){
		...
		if($n==0){
			...
			$post_ph = array(
				...
				$result_ph = $this->db->insert('savsoft_users', $post_ph);	
				$parent_id= $this->db->insert_id();
				log_message('error','ID phu huynh: '.$parent_id);
		...
		$child_id= $data_insert_hs['childid'];
		log_message('error','ID hoc sinh: '.$child_id);
		//echo json_encode($data_insert_hs);
		if($data_insert_hs['message']=="success"){
		    $childidx = $data_insert_hs['childid'];
			for($i=0; $i<3;$i++){
				...
				//$sql = "select qid,question, source from savsoft_qbank where type=2 and $multigrade and cid!=29 order by rand() limit 3";
				//Video_choice
				$sql = "select qid,question, source,points from savsoft_qbank where type=2 and deleted=0 and cid!=29 order by rand() limit 3";
				$vdtar = $this->db->query($sql)->result_array();
				if(count($vdtar)>0){
					$index_es = 0;
					$index_nm = 1%count($vdtar);
	...
	public function get_category1(){
		$data=$this->db->get('savsoft_category')->result_array();
		for($i=0; $i<count($data); $i++){
			 $sql= "select count(*) as ct from savsoft_quiz where noq>1 and deleted=0 and status=1 and gids=5 and cid=".$data[$i]['cid']; 
			 $rt = $this->db->query($sql)->result();
			 foreach ($rt as $r){
				$data[$i]['count'] = $r->ct;
			 }
			 $data[$i]['img']=base_url("upload/symbol/".$data[$i]['cid'].".png");
	...
	public function get_category_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		//Check connection_key
		$this->db->where('uid',$uid);
		...
		if($auth->num_rows()==0){
			...
			$data=$this->db->get('savsoft_category')->result_array();
			$list=$this->api_model->get_list_category();
	...
	public function get_fun_mcq1_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$offset=$_POST["offset"];
		//Check connection_key
	...
	public function get_fun_mcq2(){
		...
		if(!$user){
			...
			$this->db->select('qid, question, question1,savsoft_qbank.cid,savsoft_qbank.lid');
			$this->db->where("deleted", 0);
			$this->db->where("status", 1);
			$this->db->where("type", 2);
			$this->db->where("savsoft_qbank.cid", 29);
			$this->db->not_like('question','facebook');
			$this->db->limit(50);	
			// $this->db->offset(10*$offset);
			...
			for($i=0; $i<count($data); $i++){
				...
				//get video link
				preg_match_all('/<iframe[^>]+>/i',$data[$i]['question1'], $vidTags); 
			
				$data[$i]['question1']=$vidTags[0];
				$data[$i]['img_category']=base_url("upload/symbol/".$data[$i]['cid'].".png");
				//$data[$i]['time_ago']=$this->api_model->time_ago($data[$i]['create_date']);
	...
	public function get_assign_quiz2_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$limit=$_POST["limit"];
		$page=$_POST["page"];
	...
	public function get_assign_quiz_cl_01(){
	...
	public function get_do_assign_quiz_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$limit=$_POST["limit"];
		$page=$_POST["page"];
	...
	public function load_notification4_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$page=$_POST["page"];
		$limit=$_POST["limit"];
			...
			if(!$user){
				...
				for($i=0; $i<count($data['message']); $i++){
					...
					if(strpos($notify['content'],"đã giao một bài")!==false){
						...
						$data['message'][$i]['content'] = str_replace("."," cho ",$data['message'][$i]['content'] );
						$ari=explode("/",str_replace("window.location.href = 'https://stemup.app/index.php/quiz/validate_quizs/","",$notify['click']));
						// $ari1=str_replace("window.location.href = 'http://dev.stemup.vn/index.php/quiz/validate_quizs/","",$notify['click']);

						// log_message('error','------------>string: '.$ari[0]);
						// $ari2=explode("/",$ari1);
						// log_message('error','------------>'.$ari2[0]);
						// log_message('error','------------>'.$ari2[1]);
						$aquid=$ari[0];
						$aid=$ari[2];
	                    $data['message'][$i]['model']=$notify['model'];
						$data['message'][$i]['quid']=$aquid;
	...
	public function load_notification_assign3_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$page=$_POST["page"];
		$limit=$_POST["limit"];
	...
	public function load_notification_do_quiz2_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$page=$_POST["page"];
		$limit=$_POST["limit"];
	...
	public function read_notification_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$nid=$_POST["nid"];
		//Check connection_key
	...
	public function assignquizapi2(){
		...
		if($auth->num_rows()==0){
			...
			if($this->api_model->assignQuizApi2($user, $quid, $uid, $startdate, $enddate,$reward)){
				...
				log_message('error','-------Giao bai thanh cong');
			}else{
				$data['result'] = "Cannot assign for user.";
				log_message('error','-------Loi giao bai cho hoc sinh');
				log_message('error',$line);
			}
		}
	...
	function get_mcq_by_category_or_level_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$cid =$_POST["cid"];
		$lid =$_POST["lid"];
	...
	function get_history_hcc_assign_quiz_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$parent=$_POST["parent"];
		$childid=$_POST["uid"];
	...
	function new_hcc_assign_quiz_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		$parent=$_POST["parent"];
	...
	function get_statistic_answer_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		//Check connection_key
		$this->db->where('uid',$uid);
	...
	function get_statistic_answer1($uid){
	...
	function create_quiz_assign2($key, $distinceday=1){
		if($key=='ctv1xnvhk078j5h0c2bjq5wp1pennqoyizswcdbc'){
			...
			foreach($children as $child){
				...
				$sql = "select qid,question, source,points from savsoft_qbank where deleted=0 and status=1 and type=2 and ".$mvlid." and cid!=29 order by rand() limit 3";
				$vdtar = $this->db->query($sql)->result_array();
				if(count($vdtar)>0){
					$index_es = 0;
					$index_nm = 1%count($vdtar);
	...
	function create_quiz_assign3($key, $distinceday=1){
		if($key=='ctv1xnvhk078j5h0c2bjq5wp1pennqoyizswcdbc'){
			$distince_time="+$distinceday day";
			$today=  date("Y-m-d", strtotime($distince_time));
			$today1=  date("d-m", strtotime($distince_time));
			...
			foreach($children as $child){
				...
				log_message('error', $child['uid']."------x");
				$ac= rtrim($child['assign_categories'],',');
				$grade=$child['grade'];
...
//HCC	
	...
	function get_fun_quiz3_01(){
		...
		if($auth->num_rows()==0){
			...
			$lid_sql="select grade from savsoft_users where uid=$childid";
			$lid_ar=$this->db->query($lid_sql)->row_array();
			$lid=$lid_ar['grade'];
			...
			if($lid>=3 && $lid<=7){
				...
				//Lay du lieu recommend_assign ngay hien tai


				//Lay du lieu recomment_assign cac ngay truoc
				for($i=5*$page; $i<5*($page+1);$i++){
					$k=$i-1;
				...
				$set=0;
						...
						if($today>"2019-03-28"){
							...
							$r = array(
										 "date"=> $ardow[date('w',strtotime("-$k day"))].", ".date("d / m", strtotime("-$k day")),
										"assignments"=>array( array("level"=>"Ngắn",
										                            "quiz"=> array("quid"=>$data_es['quid'],
																	                "total_points"=>$total_point_es,
																					"number_question"=>$data_es['noq'],
	...
	function create_child_account2(){
		...
		if($data['message']=="success"){
			log_message('error','--->Them con thanh cong');
			...
			for($i=0; $i<3;$i++){
				...
				log_message('error', '------>Grade: '.$grade);
				
				if($lop>=6 && $lop<=8){
					$multigrade=16;
				}else if($lop>=9 && $lop<=11){
					$multigrade=17;
				}else if($lop>=12 && $lop<=14){
					$multigrade=18;
				}else{
					$multigrade=15;
				}

				$sql = "select qid,question, source,points from savsoft_qbank where deleted=0 and type=2 and (lid=".$grade." or lid=".$multigrade.") and cid!=29 order by rand() limit 3";
	...
	function update_avatar($uid){
	    ...
	    log_message('error','--->Cap nhat anh dai dien thanh cong');
	}
	...
	function get_child_timetable_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		//Check connection_key
		$this->db->where('uid',$uid);
	...
	function get_schedule_by_category_01(){
		$connection_key=$_POST["connection_key"];
		$uid=$_POST["uid"];
		//Check connection_key
		$this->db->where('uid',$uid);
	...
	function change_type_account_01(){
   ...
   function update_s(){
	...
	function make_quiz($childid){
				...
				$_video=$this->db->query($sql_video)->result_array();	
					...
					log_message('error',$video_question);

				preg_match_all('/<iframe[^>]+>/i',$_video[0]['question'], $vidTags); 
					if(count($vidTags[0])>0){
						 preg_match('/src="([^"]+)"/', $vidTags[0][0], $match);
						 if(strpos($match[1],'youtube')!==false){
							 $vid= substr(str_replace("https://www.youtube.com/embed/","",$match[1]),0,11);
				...
				// if(strpos($_video[0]['question'],'embed/')!==0){
				...
				// }
				// if(strpos($_video[0]['question'],'facebook')!==0){
				// 	$start = strpos($match[1],"href=");
				// 	$vidlink= substr($match[1],$start+5);
				// 	$vidlink=str_replace("%3A",":",$vidlink);
				// 	$vidlink=str_replace("%2F","/",$vidlink);
				...
				preg_match_all('/<iframe[^>]+>/i',$_video[1]['question'], $vidTags); 
					if(count($vidTags[0])>0){
						 preg_match('/src="([^"]+)"/', $vidTags[0][0], $match);
						 if(strpos($match[1],'youtube')!==false){
							 $vid= substr(str_replace("https://www.youtube.com/embed/","",$match[1]),0,11);
				...
				preg_match_all('/<iframe[^>]+>/i',$_video[2]['question'], $vidTags); 
					if(count($vidTags[0])>0){
						 preg_match('/src="([^"]+)"/', $vidTags[0][0], $match);
						 if(strpos($match[1],'youtube')!==false){
							 $vid= substr(str_replace("https://www.youtube.com/embed/","",$match[1]),0,11);
				...
				log_message('error', 'Da tao bai quiz HCC hang ngay');
	}
						...
						if($vq_es){
							...
							if(count($vidTags[0])>0){
								 ...
								 if(strpos($match[1],'youtube')!==false){
									...
									$posid=strpos($temp,"%2F");
									$vid= substr($temp,0,$posid);
									$vquestion_es_video="https://www.facebook.com/plugins/video.php?href=https%3A%2F%2Fwww.facebook.com%2Fthedadlab%2Fvideos%2F".$vid."%2F";
	...
	function delete_user(){
		...
		if($query){
			...
			log_message('error','--->Xoa con thanh cong');
		}else{
			echo json_encode(array('message'=>'Fail!','status'=>'0'));
			log_message('error','--->Xoa con that bai');
		}
	}
