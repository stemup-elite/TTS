...
class Sadmin extends CI_Controller {
	...
	function edit_quiz($qid){
		...
		$data['qbank']=$this->quiz_model->get_quiz_in($data['qids'],0,0,'',12,0);
		...
		$data['qbank_not'] = $this->quiz_model->get_quiz_not_in($data['qids'],0,0,'',12,0);
  ...
  //           $config = array();
	...
	function edit_news($id){
		...
		if($user['uid'] == 1){
			...
			if(!is_null($data['new']['related_news'])){
...
class Sadmin extends CI_Controller {
	...
	function video($page=1){
		...
		$data['stcategories']=$this->qbank_model->get_statistic_category();
		$data['post_tag']=$this->post_model->post_list($type_tag='post_tag');
		$data['video_list']=$this->library_model->get_data_library("video",$data['search'],6,$page-1);
		$data['numpage'] =ceil($this->library_model->number_data_library("video",$data['search'])/6);
		$data['page']=$page;
		$data['category_list']=$this->qbank_model->category_list();
		$data['level_list']=$this->qbank_model->level_list();
		if($data['numpage']>10){
	...
	function add_quiz(){
		$logged_in = $this->session->userdata('logged_in');
		$uid = $logged_in['uid'];
		$noq = sizeof(explode(",",$this->input->post('string_qids')));
		if($uid == 1){
		//	$data['name'] = $_POST['quiz_name'];
			$userdata=array(
				'quiz_name'=>$this->input->post('quiz_name'),
